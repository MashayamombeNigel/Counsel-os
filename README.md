...
Controllers are intentionally thin and they validate, delegate to a service, and redirect. All business logic (status transitions, activity logging, AI orchestration) lives in the service layer, which is where the actual engineering decisions are visible.
...
- **Real full-text search** across document content (Postgres `tsvector` + GIN index + `ts_headline`) not just title matching but searches the actual extracted text of every uploaded document, with highlighted result snippets
...
- Automated Pest test suite (Unit + Feature) with GitHub Actions CI running against a real Postgres service container on every push/PR and note Gemini calls are faked so nothing hits a real API key in CI
...
- **Research retrieval is intentionally simple** (insight-based context, no embeddings) accurate for MVP scope but not exhaustive for very large multi-document matters.
...
Run the app and the queue worker because extraction and AI analysis won't process without the worker running:
